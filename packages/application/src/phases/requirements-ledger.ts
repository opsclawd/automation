...
export type ArchitectureRequirementCategory = RequirementCategory;
...
export interface RequirementLedgerItem {
  ...
  hardGate?: boolean | undefined;
}

export type ArchitectureRequirementItem = RequirementLedgerItem;
...
export type ArchitectureRequirementsLedger = RequirementsLedger;
...
const HARD_GATE_REGEX =
  /\b(must|never|always|before|hash|prevent|provenance|capability|atomic|order|concurrent|idempotent|rollback)\b/i;

export function isHardGateRequirement(title: string): boolean {
...
export async function buildRequirementsLedger(
  opts: BuildRequirementsOptions,
): Promise<RequirementsLedger> {
  const items: RequirementLedgerItem[] = [];
  const seenTitles = new Set<string>();

  const addItem = (item: RequirementLedgerItem) => {
    const key = `${item.category}:${item.title.toLowerCase().trim()}`;
    if (!seenTitles.has(key)) {
      seenTitles.add(key);
      const hardGate = item.hardGate ?? isHardGateRequirement(item.title);
      items.push({
        ...item,
        hardGate,
      });
  ...
  // 3. Anchored design sections from design.md artifact (when present)
  if (opts.designMd && opts.designMd.trim().length > 0) {
    const designSections = extractSections(opts.designMd);
    let designDocIdx = 1;

    if (designSections.size > 0) {
      for (const [dHeader, dSecLines] of designSections.entries()) {
        if (
          dHeader.includes('non-goals') ||
          dHeader.includes('non goals') ||
          dHeader.includes('nongoals') ||
          dHeader.includes('traps')
        ) {
          const extracted = extractBulletsOrParagraphs(dSecLines);
          for (const entry of extracted) {
            addItem({
              id: `REQ-TRAP-${trapIdx++}`,
              category: 'trap_non_goal',
          ...
          const isDescriptive =
            dHeader.includes('problem') ||
            dHeader.includes('recap') ||
            dHeader.includes('risk') ||
            dHeader.includes('assumption') ||
            dHeader.includes('precedent') ||
            dHeader.includes('background') ||
          ...
          const isWhitelisted =
            dHeader.includes('anchored design') ||
            dHeader.includes('required changes') ||
            dHeader.includes('design') ||
            dHeader.includes('goal') ||
            dHeader.includes('requirement');

          if (isWhitelisted && !isDescriptive) {
            const extracted = extractBulletsOrParagraphs(dSecLines);
            for (const entry of extracted) {
              addItem({
                id: `DESIGN-${designDocIdx++}`,
                category: 'anchored_design',
                title: entry,
                source: 'design.md',
              });
      ...
      const designBullets = extractBulletsOrParagraphs(opts.designMd.split(/\r?\n/));
      for (const entry of designBullets) {
        addItem({
          id: `DESIGN-${designDocIdx++}`,
          category: 'anchored_design',
          title: entry,
  ...
  // 5. Two-Stage Relationship-Validated Direct Consumer Discovery
  ...
  // 6. Fallback if no specific requirements extracted
  if (items.length === 0) {
...
export const buildArchitectureRequirementsLedger = buildRequirementsLedger;

export function formatRequirementsLedgerForPrompt(ledger: RequirementsLedger): string {
  if (ledger.items.length === 0) {
    return 'No requirements recorded in ledger.';
  }

  const lines: string[] = [
    `# Requirements Ledger (Issue #${ledger.issueNumber})`,
    '',
    ...
    'For items marked [HARD GATE], you MUST include `counterexample_considered` detailing adversarial counterexample / falsification reasoning.',
    '',
  ];

  for (const item of ledger.items) {
    const hardGateTag = item.hardGate ? ' [HARD GATE]' : '';
    lines.push(
      `- **[${item.id}] [${item.category.toUpperCase()}]${hardGateTag}** (${item.source})`,
    );
