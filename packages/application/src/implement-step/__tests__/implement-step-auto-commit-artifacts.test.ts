...
import type { ImplementStepLoopDeps } from '../types.js';
...
describe('ImplementStepLoop auto-commit artifact exclusion', () => {
  it('calls git.add with dirtySourcePaths and never git.addAll when auto-committing uncommitted changes', async () => {
    ...
    const { git, addCalls, addAllCalls } = makeFakeGit({
      headSha: 'sha-pre',
      statusOutput: ' M packages/application/src/app.ts\n?? design.md\n?? task-manifest.json\n',
    });

    const deps: ImplementStepLoopDeps = {
      events: bus,
      git,
      runImplement: async () => ({ invocationId: 'i-1', agentOutcome: 'success' }),
    ...
    await loop.execute({
      ...
      stepTitle: 'step 1',
      maxIterations: 2,
      implementProfile: AgentProfileName('pi-qwen-local'),
      specReviewProfile: AgentProfileName('opencode-frontier'),
      qualityReviewProfile: AgentProfileName('opencode-frontier'),
      fixProfile: AgentProfileName('pi-qwen-local'),
    });

    expect(addAllCalls).toEqual([]);
    expect(addCalls.length).toBeGreaterThan(0);
    expect(addCalls[0].files).toEqual(['packages/application/src/app.ts']);
