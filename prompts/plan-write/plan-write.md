...
- HARD RULE — PORT/INTERFACE CHANGES: Never use `pnpm -r typecheck` (workspace-wide) as the validation for a task that adds a new method to a port or interface without ALSO updating every adapter/implementation of that interface in the same task. A workspace typecheck will fail the moment the port declares a new method that downstream adapters haven't implemented yet. Instead: use `pnpm -C <package> typecheck` scoped to the package that owns the port for that task, and assign `pnpm -r typecheck` only to the task that commits the last adapter. If a single task updates both port and all adapters, `pnpm -r typecheck` is fine.
