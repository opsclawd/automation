...
export async function composeRepositoryOperationalRuntime(
  input: ComposeRepositoryOperationalRuntimeInput,
  ...
  const checkLiveOwnership = () => {
    if (closed) return false;
    const now = new Date();
    return workerLeaseRepository.checkActiveLease(repository.id, now);
  };

  const close = () => {
    if (closed) return;
    hasActiveLease = checkLiveOwnership();
    if (hasActiveLease) {
