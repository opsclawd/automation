...
const { mockServer, mockContainer, mockSweeper, registerSpy, startDrainSpy } = vi.hoisted(() => {
  ...
  return { mockServer, mockContainer, mockSweeper, registerSpy, heartbeatSpy, startDrainSpy };
});
...
describe('cli serve sweep and worker wiring', () => {
  ...
  it('runs initial sweep and schedules periodic sweep if interval > 0', async () => {
    ...
    // Expect initial sweep was triggered
