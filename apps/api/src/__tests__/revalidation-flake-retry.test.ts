...
import type { TaskManifest, ValidationCommand, ValidationRunCommandItem, RunValidation, ValidationPort, EventBusPort } from '@ai-sdlc/application';
...
describe('maybeRetryTransientRevalidationFlake', () => {
  ...
  const mockTaskValidationCommands: ValidationCommand[] = [
...
`;
    ...
    const mockValidationAdapter = {
      run: vi.fn().mockResolvedValue([
        { command: 'pnpm --filter @ai-sdlc/other test -- out-of-scope.test.ts', outcome: 'passed' },
      ]),
    ...
    const mockRunValidation = {
      execute: vi.fn().mockResolvedValue({
        validationRun: {
          id: 'vrun-2',
          commands: [
            { command: 'pnpm -r test', outcome: 'passed', exitCode: 0 },
          ],
        },
      }),
    ...
    expect(publishedEvents[0].metadata.failedTestFiles).toEqual(['packages/other/src/__tests__/out-of-scope.test.ts']);
  });
...
`, 'utf-8');
    ...
    const mockValidationAdapter = {
      run: vi.fn().mockResolvedValue([
        { command: 'pnpm --filter @ai-sdlc/other test -- out-of-scope.test.ts', outcome: 'failed' },
      ]),
    } as unknown as ValidationPort;
