...
`,
  ...
  it('workerLoopDeps.prepareWorktree preserves existing startCommitSha and uses it as baseBranch (#1224)', async () => {
    const root = trackDir(() => mkdtempSync(path.join(os.tmpdir(), 'compose-prepare-wt-sha-')));
    writeFileSync(
      path.join(root, '.ai-orchestrator.json'),
      JSON.stringify({
        validation: { commands: ['echo ok'], timeout: 60 },
    ...
    const existingSha = '1111222233334444555566667777888899990000';
    container.runRepository.insertIfNoActive({
      uuid: 'prepare-wt-existing-sha-uuid',
    ...
    const createWorktreeSpy = vi
      ...
      .mockResolvedValue(undefined);
    vi.spyOn(GitWorktreeAdapter.prototype, 'seedArtifactExcludes').mockResolvedValue(undefined);
    const fetchSpy = vi.spyOn(GitWorktreeAdapter.prototype, 'fetch').mockResolvedValue(undefined);
    const headCommitShaSpy = vi
      .spyOn(GitWorktreeAdapter.prototype, 'headCommitSha')
      .mockResolvedValue('unexpected-different-sha');

    const ac = new AbortController();
    const result = await container.workerLoopDeps!(RepositoryId('owner/repo')).prepareWorktree({
      repoId: RepositoryId('owner/repo'),
      runId: RunId('prepare-wt-existing-sha-uuid'),
      signal: ac.signal,
    });

    expect(fetchSpy).toHaveBeenCalledWith(root, 'origin', 'release/batch-1');
    expect(createWorktreeSpy).toHaveBeenCalledOnce();
    expect(createWorktreeSpy.mock.calls[0][0]).toMatchObject({
      worktreePath: path.join(root, '.ai-worktrees', 'issue-72'),
      branch: 'ai/issue-72',
      baseBranch: existingSha,
    });
    expect(headCommitShaSpy).not.toHaveBeenCalled();

    const updated = container.runRepository.findByUuid('prepare-wt-existing-sha-uuid');
    expect(updated?.startCommitSha).toBe(existingSha);
    expect(result.cwd).toBe(path.join(root, '.ai-worktrees', 'issue-72'));
