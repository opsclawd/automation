...
export function buildImplementPrompt(
  ...
  return [
    ...
    '5. Coding & Commit Contract:',
    '   You SHOULD commit your work if possible, but if you cannot (e.g. persistent',
    '   pre-commit hook failure that you cannot resolve), you may leave the',
    '   worktree dirty and report DONE. The orchestrator will attempt to commit',
    '   for you after validation.',
    '',
    '   If you DO commit:',
    ...
    '        If you still cannot commit, proceed to step 5.d/e.',
    ...
    '   d. Never infer commit success from git log. Git log shows OTHER tasks\' commits.',
    '      Only git rev-parse HEAD + git status --porcelain prove YOUR commit landed.',
    "   e. If you cannot get the commit to land, ensure your code changes are staged",
    "      or present in the worktree before reporting DONE.",
...
export async function buildImplementStepFixPrompt(
  ...
  return [
    ...
    '  | { "result": "cannot_fix" }',
    '',
    '## CODING & COMMIT CONTRACT',
    'You SHOULD commit your change if possible, but if you cannot (e.g. persistent',
    'pre-commit hook failure), you may leave the worktree dirty and report',
    'done_with_fixes. The orchestrator will attempt to commit for you after validation.',
    '',
    'If you DO commit:',
    ...
    '     output, FIX the reported errors, and retry the commit.',
    ...
    '     `[ -z "$(git status --porcelain)" ] || { echo "WORKTREE DIRTY AFTER COMMIT"; exit 1; }`',
    '  6. If you cannot get the commit to land, ensure your code changes are staged',
    '     or present in the worktree before writing result.json.',
  ].join('\n');
}
...
export function buildPostPrReviewTaskPrompt(input: BuildPostPrReviewTaskPromptInput): string {
  ...
  sections.push(
    ...
    '2. Coding & Commit Contract:',
    '   You SHOULD commit your change if possible, but if you cannot (e.g. persistent',
    '   pre-commit hook failure), you may leave the worktree dirty and report',
    '   action=fixed. The orchestrator will attempt to commit for you after validation.',
    '',
    '   If you DO commit:',
    '   a. Record HEAD before: `PRE_HEAD=$(git rev-parse HEAD)`',
    ...
    '      `[ -z "$(git status --porcelain)" ] || { echo "WORKTREE DIRTY AFTER COMMIT"; exit 1; }`',
    '   f. If you cannot get the commit to land, ensure your code changes are staged',
    '      or present in the worktree before reporting action=fixed.',
