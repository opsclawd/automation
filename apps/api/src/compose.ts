...
export function buildImplementPrompt(
  ...
  return [
    ...
    '   b. Stage the exact implementation files: git add <files>',
    '   c. Commit with the message on stdin, using real line breaks:',
    '',
    '      ```bash',
    ...
    '      in a message; the quoted heredoc preserves the message exactly.',
    '   d. Verify the commit landed:',
    ...
    '   e. HARD RULE: Never infer commit success from git log. Git log shows',
    "      OTHER tasks' commits. Only git rev-parse HEAD + git status --porcelain",
    '      prove YOUR commit landed.',
    "   f. If you cannot get the commit to land (hook failure you can't fix),",
    ...
    '  git status --porcelain empty? (See step 5d/e.)',
...
export function buildQualityReviewPrompt(options: BuildQualityReviewPromptOptions): string {
  ...
  sections.push(
    ...
    'Rules:',
    '- "findings" MUST be present and an array (use `[]` on pass).',
    '- When result is "fail" or "fabricated", findings MUST contain at least one entry with severity P0..P3.',
    "- Every finding's `summary` is required and MUST be non-empty.",
    '- `file` and `suggested_fix` are STRONGLY RECOMMENDED for actionable findings; they may be omitted when the defect spans multiple files or is a plan/letter deviation.',
    '- Do NOT omit `findings` on "fail" — the orchestrator\'s fixer and arbiter cannot act on a fail verdict without findings.',
    '- Use "fabricated" only when the implementation presents evidence as the result of external physical execution (hardware telemetry, benchmark measurements, certifications, or comparable captures) and the reviewed provenance establishes that execution did not occur. Use ordinary "fail" for buggy, inconsistent, incomplete, or low-quality output when fabrication is not established. A "fabricated" result hard-fails the Step and is not sent to a fixer, so include concrete evidence in findings.',
    'Do NOT write to a relative path — use the full absolute path above.',
    'You MUST use your file-write tool to create result.json on disk. Printing the',
    'JSON in your chat response instead of writing the file is a contract violation —',
    'the orchestrator only reads the file, never your response text.',
