...
- **Explicit Path Matching & Precedence**:
  ...
  - `reference_files` overlap with writable declarations (`expected_files`, `may_extend`) within the *same* task is rejected at manifest-schema validation time (`taskManifestV2Schema`), not resolved by runtime precedence. Tier 6 above governs only reference files declared by a *different* task in the manifest.
