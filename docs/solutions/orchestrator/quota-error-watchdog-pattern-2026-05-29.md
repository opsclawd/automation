...
**Fix:** ~~Scan ALL `.log` files with `mtime >= startTime`. Per-file offsets tracked via `Map<string, number>`. The deeper concern about correlating to the correct child process remains unresolved (first quota pattern across any file still kills) — accepted as a single-tenant assumption.~~ **Resolved (Issue #191):** Each invocation now creates a per-invocation `session-log/` directory inside its artifact directory and passes it to the child via `OPENCODE_SESSION_LOG_DIR`. The watchdog and post-exit scanner read only from this directory, so cross-worker contamination is impossible by construction. The `sessionLogDir` option, mtime filtering, and `captureInitialSessionLogOffsets` have been removed.
